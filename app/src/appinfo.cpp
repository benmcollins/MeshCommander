...
{
    // Q_PROPERTY(... CONSTANT) means the QML engine assumes this
    // returns the same value on every read. The body opens a
    // resource file, parses Markdown via QTextDocument, and walks
    // the styled HTML out — non-trivial work the AboutDialog binding
    // used to pay per open. Cache the result in a function-local
    // static (thread-safe init since C++11). See #285.
    static const QString cached = []() -> QString {
        ...
        // Convert the Markdown source to HTML via `QTextDocument` so
        // we can hand the result to `TextEdit.RichText` instead of
        // relying on the AboutDialog's body font scaling. Qt's
        // `MarkdownText` mode honoured the inherited (small)
        // pixelSize and ended up rendering the document flat —
        // headings barely distinguishable from the body. Forcing an
        // explicit stylesheet here pins the hierarchy regardless of
        // the QML font settings.
        QTextDocument doc;
